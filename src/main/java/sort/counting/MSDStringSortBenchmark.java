...
public class MSDStringSortBenchmark {
    final static LazyLogger logger = new LazyLogger(MSDStringSortBenchmark.class);
    ...
    private void doSortStrings(final int n, final int m) {
        benchmarkStringSorters(COMMON_CHINESE_WORDS_CORPUS, MSDStringSortBenchmark.getWords(COMMON_CHINESE_WORDS_CORPUS, MSDStringSortBenchmark::lineAsList), n, m,new PinyinHelper());
    }
    ...
    void benchmarkStringSorters(final String corpus, final String[] words, final int nWords, final int nRuns, Helper helper) {
        ...
        final boolean purMSDStringSort = false;
        final MSDStringSort msdStringSort = new MSDStringSort();
        final String s1 = "MSDStringSort" + (purMSDStringSort ? " with insertion sort" : "");
        final Benchmark<String[]> benchmark = new Benchmark<>(getDescription(nWords, s1, s2), null, MSDStringSort::sort, null);
    ...
    static String[] getWords(final String resource, final Function<String, List<String>> stringListFunction) {
        try {
            final File file = new File(getPathname(resource, MSDStringSort.class));
